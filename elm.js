...
Elm.Types.make = function (_elm) {
   ...
   return _elm.Types.values = {_op: _op,Alive: Alive,Dead: Dead};
};
Elm.Game = Elm.Game || {};
Elm.Game.make = function (_elm) {
   ...
   var _op = {};
   var unwrap = function (maybe) {
      var _p0 = maybe;
      if (_p0.ctor === "Just") {
            return _p0._0;
         } else {
            return _U.crashCase("Game",{start: {line: 76,column: 3},end: {line: 81,column: 62}},_p0)("Invalid coordinates resulted in Nothing");
         }
   };
   var wrapAround = F3(function (limit,value,offset) {
      var off = A2($Basics._op["%"],value + offset,limit);
      return _U.cmp(off,0) < 0 ? limit + off : A2($Basics._op["%"],off,limit);
   });
   var wrappedOffset = F3(function (m,offset,original) {
      return {ctor: "_Tuple2"
             ,_0: A3(wrapAround,$Matrix.rowCount(m),$Matrix.row(original),$Matrix.row(offset))
             ,_1: A3(wrapAround,$Matrix.colCount(m),$Matrix.col(original),$Matrix.col(offset))};
   });
   var neighbours = F2(function (loc,matrix) {
      return A2($List.map,
      unwrap,
      A2($List.map,
      function (loc) {
         return A2($Matrix.get,loc,matrix);
      },
      A2($List.map,
      A2(wrappedOffset,matrix,loc),
   ...
   var stepByNeighbors = F2(function (state,neighs) {
      var living = $List.length(A2($List.filter,function (m) {    return _U.eq(m,$Types.Alive);},neighs));
      return _U.eq(state,$Types.Alive) ? _U.eq(living,2) || _U.eq(living,3) ? $Types.Alive : $Types.Dead : _U.eq(living,3) ? $Types.Alive : $Types.Dead;
   });
   var stepLocation = F3(function (matrix,location,current) {    var neighs = A2(neighbours,location,matrix);return A2(stepByNeighbors,current,neighs);});
   var step = function (m) {    return A2($Matrix.mapWithLocation,stepLocation(m),m);};
   return _elm.Game.values = {_op: _op,step: step};
};
...
Elm.GameView.make = function (_elm) {
   ...
   $Types = Elm.Types.make(_elm);
   var _op = {};
   var cellStyle = function (s) {
      var _p0 = s;
      if (_p0.ctor === "Alive") {
            return $Html$Attributes.$class("gol-alive");
         } else {
            return $Html$Attributes.$class("gol-dead");
         }
   };
   var cell = function (c) {    return A2($Html.td,_U.list([cellStyle(c)]),_U.list([$Html.text(" ")]));};
   var row = function (r) {    return A2($Html.tr,_U.list([]),$Array.toList(A2($Array.map,cell,r)));};
   var gameView = function (world) {
      return A2($Html.table,_U.list([$Html$Attributes.$class("gol-map")]),_U.list([A2($Html.tbody,_U.list([]),$Array.toList(A2($Array.map,row,world)))]));
   };
   return _elm.GameView.values = {_op: _op,gameView: gameView};
};
...
Elm.GameOfLife.make = function (_elm) {
   ...
   $GameView = Elm.GameView.make(_elm),
   $Html = Elm.Html.make(_elm),
   $Html$Attributes = Elm.Html.Attributes.make(_elm),
   $Html$Events = Elm.Html.Events.make(_elm),
   $List = Elm.List.make(_elm),
   $Matrix = Elm.Matrix.make(_elm),
   $Matrix$Random = Elm.Matrix.Random.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   ...
   $Types = Elm.Types.make(_elm);
   var _op = {};
   var generateRandomWorld = function (initialSeed) {
      return $Basics.fst(A2($Random.generate,
      A3($Matrix$Random.matrix,
      A2($Random.$int,25,25),
      A2($Random.$int,25,25),
      A2($Random.map,function (b) {    return b ? $Types.Alive : $Types.Dead;},$Random.bool)),
      initialSeed));
   };
   var SeedInputChanged = function (a) {    return {ctor: "SeedInputChanged",_0: a};};
   var Reset = {ctor: "Reset"};
   var Tick = function (a) {    return {ctor: "Tick",_0: a};};
   var Stop = {ctor: "Stop"};
   var Start = {ctor: "Start"};
   var StepOne = {ctor: "StepOne"};
   var Model = F6(function (a,b,c,d,e,f) {    return {world: a,previousWorld: b,seed: c,mode: d,round: e,seedInput: f};});
   var Complete = {ctor: "Complete"};
   var simulationButton = F2(function (address,model) {
      var _p0 = function () {
         var _p1 = model.mode;
         if (_p1.ctor === "Manual") {
               return {ctor: "_Tuple2",_0: Start,_1: "Start"};
            } else {
               return {ctor: "_Tuple2",_0: Stop,_1: "Stop"};
            }
      }();
      var action = _p0._0;
      var text = _p0._1;
      return A2($Html.button,
      _U.list([A2($Html$Events.onClick,address,action),$Html$Attributes.disabled(_U.eq(model.mode,Complete))]),
      _U.list([$Html.text(text)]));
   });
   var Automatic = {ctor: "Automatic"};
   var Manual = {ctor: "Manual"};
   var initialModel = function (s) {
      var seed = $Random.initialSeed(s);
      return {world: generateRandomWorld(seed),previousWorld: $Maybe.Nothing,seed: s,mode: Manual,round: 0,seedInput: $Maybe.Nothing};
   };
   var init = {ctor: "_Tuple2",_0: initialModel(42),_1: $Effects.none};
   var stepOneButton = F2(function (address,model) {
      return A2($Html.button,
      _U.list([A2($Html$Events.onClick,address,StepOne),$Html$Attributes.disabled(!_U.eq(model.mode,Manual))]),
   ...
   var view = F2(function (address,m) {
      ...
      _U.list([A2($Html.dl,
              _U.list([]),
              _U.list([A2($Html.dt,_U.list([]),_U.list([$Html.text("Round")])),A2($Html.dd,_U.list([]),_U.list([$Html.text($Basics.toString(m.round))]))]))
              ,$GameView.gameView(m.world)
              ,A2(stepOneButton,address,m)
              ,A2(simulationButton,address,m)
              ...
              _U.list([$Html$Attributes.type$("number")
                      ,$Html$Attributes.value($Basics.toString(A2($Maybe.withDefault,m.seed,m.seedInput)))
                      ,A3($Html$Events.on,
                      "input",
                      $Html$Events.targetValue,
                      function (input) {
                         return A2($Signal.message,address,SeedInputChanged(input));
                      })]),
              _U.list([]))]));
   });
   var update = F2(function (action,m) {
      var _p2 = action;
      switch (_p2.ctor)
      {case "StepOne": var next = $Game.step(m.world);
           var continuingModel = _U.update(m,{world: next,previousWorld: $Maybe.Just(m.world),round: m.round + 1});
           var _p3 = m.previousWorld;
           if (_p3.ctor === "Nothing") {
                 return {ctor: "_Tuple2",_0: continuingModel,_1: $Effects.tick(Tick)};
              } else {
                 return !_U.eq(_p3._0,next) ? {ctor: "_Tuple2",_0: continuingModel,_1: $Effects.tick(Tick)} : {ctor: "_Tuple2"
                                                                                                              ,_0: _U.update(m,{mode: Complete})
                                                                                                              ,_1: $Effects.none};
              }
         case "Start": return {ctor: "_Tuple2",_0: _U.update(m,{mode: Automatic}),_1: $Effects.tick(Tick)};
         case "Stop": return {ctor: "_Tuple2",_0: _U.update(m,{mode: Manual}),_1: $Effects.none};
         case "Tick": var _p4 = m.mode;
           if (_p4.ctor === "Automatic") {
                 return {ctor: "_Tuple2",_0: m,_1: $Effects.task($Task.succeed(StepOne))};
              } else {
                 return {ctor: "_Tuple2",_0: m,_1: $Effects.none};
              }
         case "SeedInputChanged": return {ctor: "_Tuple2",_0: _U.update(m,{seedInput: $Result.toMaybe($ParseInt.parseInt(_p2._0))}),_1: $Effects.none};
         default: var mode = !_U.eq(m.mode,Complete) ? m.mode : Manual;
           var fx = _U.eq(mode,Manual) ? $Effects.none : $Effects.tick(Tick);
           var fresh = initialModel(A2($Maybe.withDefault,m.seed,m.seedInput));
           return {ctor: "_Tuple2",_0: _U.update(fresh,{mode: mode}),_1: fx};}
   });
   ...
   return _elm.GameOfLife.values = {_op: _op
                                   ...
                                   ,Tick: Tick
                                   ,Reset: Reset
                                   ,SeedInputChanged: SeedInputChanged
                                   ,init: init
                                   ,initialModel: initialModel
                                   ,generateRandomWorld: generateRandomWorld
                                   ,view: view
